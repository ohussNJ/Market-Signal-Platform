...
Each ticker has its own tab with sub-chart tabs:

| Tab | Price panel | Indicator panel |
|-----|-------------|-----------------|
| **Score** | Candlestick + EMA/SMA 50/100/200 + KC + NW + Bull Band | Smoothed score histogram with BULL/BEAR coloring |
| **RSI** | Same as Score | RSI(14) + RSI MA, OB/OS zones, divergence lines |
| **Stoch RSI** | Same as Score | Slow StochRSI (3,3,14,14) and Fast StochRSI (3,3,6,9) |
| **OBV** | Same as Score | OBV + OBV EMA, divergence lines |
| **Keltner** | Candlestick + Keltner Channel + NW Envelope (dotted) + 17-bar and 42-bar reference lines | CNV_TB histogram (blue above zero, red below) |
...
|-----------|-----------|----------------|----------------|------------|
| RSI | Period 14, MA 14 | Slope up AND RSI > MA; OR RSI >= 56 | Slope down AND RSI < MA; OR RSI <= 36 | +/-20 |
...
| OBV | EMA 20, slope 20 bars | OBV > EMA AND slope up | OBV < EMA AND slope down | +/-10 |
...
| **NEUTRAL** | Default state | N/A |

---

## Data source

Market data is pulled from **Yahoo Finance** via the `yfinance` library, no API key required.
...
- **Hourly data**: ~60 days of history, fetched on demand when switching to the 3H interval
- Prices are **split and dividend adjusted** automatically
- Daily/weekly data is end-of-day. Crypto (BTC/ETH) ticks 24/7 so has no weekend gaps.
...
Python was not added to PATH during install. Uninstall Python and reinstall, and make sure to tick **"Add python.exe to PATH"** on the first installer screen.
...
```
If that does not work, check for a newer release at https://github.com/ranaroussi/yfinance

**Data shows N/A for some indicators**
Normal for the first few bars of a series due to indicator warm-up periods (e.g. a 200-period EMA needs 200 bars before it has a value). Also expected on the 3H interval where fewer than 200 bars are available for equities.

**App is slow to open**
On first launch it fetches 2 years of daily + 5 years of weekly data for all tickers. Subsequent refreshes use a 4-hour disk cache so they are faster.
